    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-xl bg-surface-raised border hairline-border space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-white">Sync Bundled Content</h3>
            <p className="text-xs text-white/40 mt-0.5">
              Write the built-in templates, lenses, stickers and the {BUILT_IN_FONTS.length}-font
              library into Firestore so they become editable records here. Safe to re-run.
            </p>
          </div>
          <button
            onClick={async () => {
  ...
  return (
    <Routes>
      <Route element={<AdminLayout />}>
        ...
        {/* Stat design is part of the template editor now. */}
        <Route path="stat-designs" element={<Navigate to="/admin/templates" replace />} />
        {/* Lens design lives in Studio now. */}
        <Route path="lenses" element={<Navigate to="/admin/studio" replace />} />
        <Route path="stickers" element={<StickerManager />} />
        {/* Stock photos live in the Library's Photos tab. */}
        <Route path="stock-photos" element={<Navigate to="/admin/library" replace />} />
        <Route path="filters" element={<LensFilterManager />} />
        {/* Fonts, palettes and photos are all managed in the Library's tabs.
            Redirects rather than removals so old bookmarks still land. */}
        <Route path="fonts" element={<Navigate to="/admin/library" replace />} />
        <Route path="color-palettes" element={<Navigate to="/admin/library" replace />} />
